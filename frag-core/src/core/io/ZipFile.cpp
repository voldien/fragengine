...
IO *ZipFile::openFile(unsigned int index) {
	struct zip_file *zfile;
	struct zip_stat stat;
	char buf[1024];
	int err;
	...
	if (err != 0) {
		zip_error_to_str(buf, sizeof(buf), err, errno);
		throw InvalidArgumentException(fvformatf("can't open `%d':\n\t%s\n", index, buf));
	}
	...
	if (!zfile) {
		...
		zip_error_to_str(buf, sizeof(buf), err, errno);
		throw InvalidArgumentException(fvformatf("can't open index `%d':\n\t%s\n", index, buf));
	}
...
std::vector<std::string> ZipFile::listFiles(const char *path) const {
	...
	if (index < 0)
		throw InvalidArgumentException(fvformatf("%s", path));
...
std::vector<std::string> ZipFile::listDirectories(const char *path) const {
	int err;
	struct zip *zip;
	ZipFile *zipfile;
	char buf[4096];
...
std::vector<std::string> ZipFile::list(const char *path) const {
...
void ZipFile::closeFile(IO *io) {

}

void ZipFile::remove(const char *path) {
	int err;
	char buf[4096];
	...
	if (err != ZIP_ER_OK) {
		zip_error_to_str(buf, sizeof(buf), err, errno);
		throw InvalidArgumentException(fvformatf("Could not delete file %s, error: %s", path, buf));
	}
}

void ZipFile::rename(const char *oldPath, const char *newPath) {

}

void ZipFile::createFile(const char *path) {
	zip_int64_t error = zip_add((zip_t *) this->pzip, path, NULL);
}

void ZipFile::createDirectory(const char *path) {
	zip_int64_t error = zip_dir_add((zip_t *) this->pzip, path, 0);
}

bool ZipFile::isReadable(const char *path) const {
	//err = zip_file_get_external_attributes((zip_t*)zip, index, 0, &osSys, &attr);
	return false;
}

bool ZipFile::isWriteable(const char *path) const {
	//err = zip_file_get_external_attributes((zip_t*)zip, index, 0, &osSys, &attr);
	return false;
}

bool ZipFile::exists(const char *path) const {
	struct zip_file *zfile;
	struct zip_stat stat;
	int err;
...
ZipFile *ZipFile::createZipFileObject(const char *cfilename, Ref<IScheduler> ref)
{
	...
	if (zip == NULL) {
		...
		zip_error_to_str(buf, sizeof(buf), err, errno);
		if (err == ZIP_ER_NOENT) {
			throw InvalidArgumentException(fvformatf("can't open zip archive %s - %s", cfilename, buf));
		} else {
...
static zip_int64_t io_callback(void *userdata, void *data, zip_uint64_t len, zip_source_cmd_t cmd) {
	...
	switch (cmd) {
		...
		case ZIP_SOURCE_STAT: {
			...
			st->valid = ZIP_STAT_MTIME | ZIP_STAT_SIZE | ZIP_STAT_COMP_SIZE | ZIP_STAT_COMP_METHOD |
...
ZipFile::ZipFile(const ZipFile &other) {
	this->pzip = NULL;
}
