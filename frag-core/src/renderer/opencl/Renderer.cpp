...
cl_context createCLContext(int *ndevices, cl_device_id **devices, SDL_Window *window, SDL_GLContext glcontext) {
...
#ifndef MDHEADLESS
			glcontext ? CL_GL_CONTEXT_KHR : 0, (cl_context_properties) glcontext,
#ifdef SDL_VIDEO_DRIVER_X11
			glcontext ? CL_GLX_DISPLAY_KHR : 0, glcontext ? (cl_context_properties) sysinfo.info.x11.display : NULL,
#endif
#endif
			(cl_context_properties) NULL, NULL,
	};
	...
	if (ciErrNum != CL_SUCCESS)
		throw RuntimeException(fvformatf("failed to get number of OpenCL platforms - %s\n", getCLStringError(ciErrNum)));
	...
	if (ciErrNum != CL_SUCCESS)
		throw RuntimeException(fvformatf("failed to get OpenCL platforms - %s\n", getCLStringError(ciErrNum)));
		...
		*/
		...
		if (glcontext && supportgl(extenions)) {
			...
			if (clGetGLContextInfoKHR == NULL) {
				throw RuntimeException(fvformatf("clGetExtensionFunctionAddressForPlatform failed\n"));
				return NULL;
			}
			...
			ciErrNum = clGetGLContextInfoKHR(props, CL_CURRENT_DEVICE_FOR_GL_CONTEXT_KHR, sizeof(gpuShareId),
			                                 ...
			                                 &pvar);
			if (ciErrNum != CL_SUCCESS) {
				throw RuntimeException(fvformatf("failed to get OpenGL context info - %s\n", getCLStringError(ciErrNum)));
				return NULL;
			}
		} else {
			...
			if (ciErrNum != CL_SUCCESS) {
				throw RuntimeException(fvformatf("failed to get Device ID number - %s\n", getCLStringError(ciErrNum)));
				return NULL;
			}
			...
			*devices = (cl_device_id *) malloc(sizeof(cl_device_id) * *ndevices);
			...
			if (ciErrNum != CL_SUCCESS) {
				throw RuntimeException(fvformatf("failed to get Device ID poiners - %s\n", getCLStringError(ciErrNum)));
				return NULL;
			}
			...
			for (y = 0; y < *ndevices; y++) {
				...
				clGetDeviceInfo(*devices[y], CL_DEVICE_IMAGE_SUPPORT, sizeof(cl_bool),
				                &imageSupport, NULL);
	...
	if (context == NULL || ciErrNum != CL_SUCCESS) {
		throw RuntimeException(fvformatf("failed to create OpenCL context - %s\n", getCLStringError(ciErrNum)));
		return NULL;
	}
...
cl_program createProgram(cl_context context, unsigned int nDevices, cl_device_id *device, const char *source) {
	...
	if (device == NULL || nDevices < 1) {
		throw RuntimeException(fvformatf("Failed to create program CL shader - \n %s", getCLStringError(ciErrNum)));
	}
	...
	if (ciErrNum != CL_SUCCESS) {
		if (ciErrNum == CL_BUILD_PROGRAM_FAILURE) {
			...
			for (i = 0; i < nDevices; i++) {
				...
				/*	Throw error,	*/
				throw RuntimeException(fvformatf("failed to compile CL shader - %s - %s\n", (const char *) build_log,
				                                 getCLStringError(ciErrNum)));
			}

		} else {
			/*  */
			throw RuntimeException(fvformatf("failed to compile CL shader - %s", getCLStringError(ciErrNum)));
		}
	}
...
cl_kernel createKernel(cl_program program, const char *name) {
	...
	if (ciErrNum != CL_SUCCESS || !kernel) {
		throw RuntimeException(
				fvformatf("failed to create OpeNCL kernel from program - %s\n", getCLStringError(ciErrNum)));
		return NULL;
	}
	return kernel;
}
...
cl_command_queue createCommandQueue(cl_context context, cl_device_id device) {
	...
	/*  Check error.    */
	if (ciErrNum != CL_SUCCESS)
		throw RuntimeException(fvformatf("failed to create command queue - %s\n", getCLStringError(ciErrNum)));
...
cl_mem createWTextureMem(cl_context context, int width, int height) {
	...
	if (ciErrNum != CL_SUCCESS)
		throw RuntimeException(fvformatf("clCreateImage failed: %s\n", getCLStringError(ciErrNum)));
